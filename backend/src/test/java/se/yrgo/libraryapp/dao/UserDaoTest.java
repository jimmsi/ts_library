...
public class UserDaoTest {
    ...
    void getExistingLoginInfo() throws SQLException {
        ...
        when(stmt.executeQuery(anyString())).thenReturn(rs);  when(rs.next()).thenReturn(true, false);
        when(rs.getInt("id")).thenReturn(id.getId());
        when(rs.getString("password_hash")).thenReturn(passwordHash);
        UserDao userDao = new UserDao(ds);
        LoginInfo info = userDao.getLoginInfo("1").get();  assertThat(info.getUserId()).isEqualTo(id);
